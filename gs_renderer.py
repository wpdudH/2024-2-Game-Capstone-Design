...
from DMTet_utils import return_mesh
...
class GaussianModel:
    ...
    # # GaussianModel 클래스 def extract_fields 밑에 추가 & 수정
    # def extract_point_cloud(self, occ, density_thresh=1.0):#포인트 클라우드 생성을 위한 함수 추가하기
    #     """
    #     밀도 임계값(density_thresh)을 기준으로 포인트 클라우드 생성. 
    #     / 근데 이런식으로 퉁쳐도 되는진 모르겠다
    #     """
    #     points = []
    ...
    #                 if occ[x, y, z] >= density_thresh:
    #                     points.append([
    #                         x / (resolution - 1.0) * 2 - 1,
    #                         y / (resolution - 1.0) * 2 - 1,
    #                         z / (resolution - 1.0) * 2 - 1
    #                     ])
    ...
    #     points = self.extract_point_cloud(occ, density_thresh) #밀도 임계값을 기반으로 포인트 클라우드 생성

    #     v, f = return_mesh(points) #DMtet으로 메쉬 생성
...
class Renderer:
    ...
    def initialize(self, input=None, num_pts=5000, radius=0.5):
        ...
        if input is None:
            ...
            # points 필드로 coords 배열 할당
            points = input[0].coords # coords가 리스트가 아니라 numpy array로 제공되는 경우
                
            # 색상 채널 R, G, B를 쌓아서 colors 배열 생성
            colors = np.stack((input[0].channels['R'], 
                                input[0].channels['G'], 
                                input[0].channels['B']), axis=-1)
                
            # 노말이 주어지지 않은 경우, 빈 배열로 설정
            normals = np.zeros_like(points)  # 노말을 계산하고 싶으면 이 부분을 수정
                
            # 리스트를 배열로 결합하여 최종 BasicPointCloud 생성
