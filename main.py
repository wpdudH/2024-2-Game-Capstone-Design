...
def get_pos_neg_text_embeddings(embeddings, azimuth_val, opt):
    if azimuth_val >= -90 and azimuth_val < 90:
        ...
        text_z = torch.cat([pos_z, embeddings['front'], end_z], dim=0) #end_z = embedding[side]
        if r > 0.8:
            front_neg_w = 0.0
        else:
        ...
        if random.random() < 0.3:
            r = r + random.gauss(0, 0.08)
        pos_z = r * start_z + (1 - r) * end_z
        text_z = torch.cat([pos_z, start_z, embeddings['front']], dim=0)
...
class GUI:
    def __init__(self, opt):
        ...
        self.log_file = os.path.join(opt.outdir, f"{opt.save_path}.txt")  # 로그 파일 경로
        ...
        #self.pointcloud = pointcloud.initialize_pointcloud(self.prompt)
        ...
        if self.opt.load is not None:
            ...
            #self.renderer.initialize(self.pointcloud, num_pts=self.opt.num_pts)
            self.renderer.initialize(self.opt.load, num_pts=self.opt.num_pts)
    ...
    def train_step(self):
        ...
        for _ in range(self.train_steps):
            ...
            if self.step in [100, 500]:
                azimuths = [0, 90, -90, 180]
                for azimuth in azimuths:
                    self.save_projection_image(self.step, azimuth)
            ...
            if self.enable_sd and self.opt.perpneg:
                text_z = [self.guidance_sd.embeddings["uncond"]] * len(hors)

                text_z_comp, weights = adjust_text_embeddings(self.guidance_sd.embeddings, hors, self.opt)
            ...
            if self.enable_sd:
                ...
                elif self.opt.perpneg:
                    loss = loss + self.opt.lambda_sd * self.guidance_sd.train_step_perpneg(text_z, weights, images, step_ratio=step_ratio if self.opt.anneal_timestep else None, trainStep=self.train_steps)
                else:
    ...
    @torch.no_grad()
    def save_projection_image(self, step, azimuth):
        """
        특정 각도(azimuth)를 기준으로 이미지를 렌더링하고 저장하며 로그를 기록합니다.
        """
        message = f"[INFO] Saving projection image at step {step}, azimuth {azimuth}..."
        self.log_message(message)

        # 카메라를 설정 (elevation은 고정, azimuth 변경)
        ...
        # 렌더링
        out = self.renderer.render(cur_cam, self.gaussain_scale_factor)
        image = out["image"].clamp(0, 1)  # 이미지 [0, 1] 범위로 클램프
        image = image.permute(1, 2, 0).cpu().numpy()

        # 저장 경로 설정
        save_path = os.path.join(self.opt.outdir, f"{self.opt.save_path}_projection_step_{step}_azimuth_{azimuth}.png")
        os.makedirs(self.opt.outdir, exist_ok=True)

        # OpenCV를 사용해 저장
        image_bgr = (image[..., ::-1] * 255).astype(np.uint8)  # RGB -> BGR, 0-255 변환
        cv2.imwrite(save_path, image_bgr)

        self.log_message(f"[INFO] Projection image saved to {save_path}")
